...
fn main() {
    ...
    let xs = all_angles.clone().iter().map(|theta| circle_radius * theta.cos()).collect::<Vec<f64>>();
    let ys = all_angles.iter().map(|theta| circle_radius * theta.sin()).collect::<Vec<f64>>();
